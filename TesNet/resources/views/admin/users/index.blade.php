    ...
    <div class="md:hidden space-y-3">
        @foreach ($users as $u)
            <div class="admin-card rounded-xl p-4 soft-shadow flex justify-between items-center gap-2">
                ...
                <a href="{{ route('admin.users.edit', $u) }}" class="shrink-0 text-primary dark:text-primary-fixed-dim font-label-sm">Edit</a>
            </div>
        @endforeach
    </div>
            ...
            <tbody class="text-on-surface dark:text-inverse-on-surface">
                @foreach ($users as $u)
                    <tr class="border-t border-outline-variant/20">
                        ...
                        <td class="p-3"><a href="{{ route('admin.users.edit', $u) }}" class="text-primary dark:text-primary-fixed-dim hover:underline">Edit</a></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
