...
@section('title', 'Edit Package — TESNET Admin')
...
<div class="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 max-w-2xl w-full mr-auto">
    ...
    <h1 class="font-headline-lg-mobile text-headline-lg-mobile text-on-surface dark:text-inverse-on-surface mb-2">Edit package</h1>
    <p class="font-label-sm admin-card-muted mb-6">Slug: <code class="text-primary dark:text-primary-fixed-dim">{{ $package->slug }}</code> (updates if you rename)</p>
    <form method="POST" action="{{ route('admin.packages.update', $package) }}" class="admin-card rounded-xl p-5 sm:p-8 soft-shadow border border-outline-variant/20 space-y-6">
        @csrf @method('PUT')
